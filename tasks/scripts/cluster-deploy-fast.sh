...
# Checks the running container first, then HostConfig (works on stopped containers).
# Falls back to 5000 if the container is not found.
...
if [[ "${#pushed_images[@]}" -gt 0 ]]; then
  ...
  if [[ "${CONTAINER_CMD}" = "podman" ]]; then
    _push_tls_args=(--tls-verify=false)
  fi
  for image_ref in "${pushed_images[@]}"; do
    "${CONTAINER_CMD}" push ${_push_tls_args[@]+"${_push_tls_args[@]}"} "${image_ref}"
  done
  push_end=$(date +%s)
